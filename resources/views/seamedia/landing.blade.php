...
<head>
  ...
  <style>
    :root{
      ...
      --teal:#0d9488;--teal-d:#0f766e;--teal-l:#14b8a6;--blue:#2563eb;--blue-d:#1d4ed8;--cyan:#22d3ee;
      --tint:rgba(13,148,136,.08);--tint-2:rgba(13,148,136,.16);
      --grad:linear-gradient(115deg,#0d9488 0%,#0ea5b7 45%,#2563eb 100%);--ok:#16a34a;
    ...
    *{margin:0;padding:0;box-sizing:border-box}html{scroll-behavior:smooth}
    ...
    body.lock{overflow:hidden}a{color:inherit;text-decoration:none}ul{list-style:none}
    ...
    .avatars span:nth-child(1){background:linear-gradient(135deg,#14b8a6,#0f766e)}.avatars span:nth-child(2){background:linear-gradient(135deg,#22d3ee,#0ea5b7)}.avatars span:nth-child(3){background:linear-gradient(135deg,#3b82f6,#1d4ed8)}.avatars span:nth-child(4){background:linear-gradient(135deg,#06232b,#0f3a45);font-size:11px}
    .trust-text strong{display:block;font-family:var(--display);color:var(--ink);font-size:15px}.trust-text span{font-size:13px;color:var(--muted)}
    ...
    .win-row{display:flex;justify-content:space-between;align-items:center;margin-bottom:18px}.win-row h4{font-size:16px}
    ...
    .float-card strong{display:block;font-family:var(--display);font-size:14px;color:var(--ink);line-height:1.2}.float-card span{font-size:12px;color:var(--muted)}
    ...
    .syn-x span{width:60px;height:60px;border-radius:50%;background:var(--grad);display:grid;place-items:center;color:#fff;box-shadow:0 14px 30px rgba(13,148,136,.35)}.syn-x svg{width:26px;height:26px}
    .services{background:var(--soft)}
    .svc-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:24px}
    .svc{background:#fff;border:1px solid var(--line);border-radius:var(--radius-lg);padding:32px;box-shadow:var(--shadow-sm);transition:var(--t)}
    ...
    .svc-ic svg{width:25px;height:25px}.svc h3{font-size:19px;margin-bottom:9px}.svc p{font-size:14px}
    /* showcase gallery */
    .filter-row{display:flex;flex-wrap:wrap;gap:10px;justify-content:center;margin-bottom:40px}
    .filter-row a{font-family:var(--display);font-size:13.5px;font-weight:600;padding:9px 16px;border-radius:99px;border:1px solid var(--line-2);color:var(--ink-2);background:#fff;cursor:pointer;transition:var(--t)}
    .filter-row a.active,.filter-row a:hover{background:var(--grad);color:#fff;border-color:transparent}
    .sc-grid{display:grid;grid-template-columns:repeat(3,1fr);gap:24px}
    .sc{background:#fff;border:1px solid var(--line);border-radius:var(--radius-lg);overflow:hidden;box-shadow:var(--shadow-sm);transition:var(--t)}
    .sc:hover{transform:translateY(-6px);box-shadow:var(--shadow-lg)}
    .sc-thumb{height:188px;background-size:cover;background-position:top center;background-color:var(--soft-2);position:relative;display:flex;align-items:flex-start;padding:14px}
    .sc-thumb .badge{font-family:var(--display);font-size:11px;font-weight:700;color:#fff;background:rgba(6,35,43,.5);padding:5px 11px;border-radius:99px}
    .sc-body{padding:22px}
    .sc-body .cat{font-size:12.5px;color:var(--muted);display:block;margin-bottom:4px}
    .sc-body h3{font-size:18px;margin-bottom:16px}
    ...
    .pkg h3{font-size:23px;margin-bottom:12px}.pkg p{font-size:14px;margin-bottom:22px;min-height:64px}
    ...
    .feat-card .ic svg{width:23px;height:23px}.feat-card h3{font-size:18px;margin-bottom:12px}
    ...
    .contact-card .row strong{font-family:var(--display);font-size:15px;color:var(--ink);display:block}.contact-card .row span{font-size:13px;color:var(--muted)}
    ...
    .foot-col a{display:block;font-size:14px;color:#9fc0c6;padding:6px 0;transition:var(--t)}.foot-col a:hover{color:#fff;padding-left:4px}
    ...
    .reveal{opacity:0;transform:translateY(28px);transition:opacity .7s var(--ease),transform .7s var(--ease)}.reveal.in{opacity:1;transform:none}
    ...
    @media (max-width:1080px){.hero-grid{grid-template-columns:1fr;gap:48px}.hero-vis{max-width:520px;margin-inline:auto;width:100%}
      ...
      .svc-grid,.pkg-grid,.sc-grid{grid-template-columns:1fr 1fr}.proc-grid{grid-template-columns:1fr 1fr}}
    ...
    @media (max-width:640px){.section{padding:74px 0}.hero{padding-top:calc(var(--nav-h) + 44px)}.hero h1{font-size:clamp(31px,9vw,42px)}
      .hero-cta{flex-direction:column}.hero-cta .btn{width:100%}.svc-grid,.pkg-grid,.sc-grid,.proc-grid{grid-template-columns:1fr}
      .stats-grid{grid-template-columns:1fr;gap:30px}.stat::after{display:none}.foot-grid{grid-template-columns:1fr}.cta-wrap,.quote-wrap{padding:44px 24px}}
  </style>
</head>
<body id="top">
  ...
  <nav class="nav" id="nav">
    <div class="wrap nav-in">
      ...
      <div class="nav-right">
        <a href="{{ $wa }}" class="btn btn-grad" target="_blank" rel="noopener">Konsultasi Gratis</a>
  ...
  <div class="drawer" id="drawer">
    <a href="#kolaborasi">Kolaborasi</a><a href="#layanan">Layanan</a><a href="#showcase">Lihat Website</a><a href="#paket">Paket</a><a href="#kontak">Kontak</a>
    <a href="{{ $wa }}" class="btn btn-grad" target="_blank" rel="noopener" style="margin-top:10px">Konsultasi Gratis</a>
  </div>
  ...
  <header class="hero">
    <div class="wrap hero-grid">
      <div class="reveal">
        <span class="eyebrow"><span class="pip"></span> {{ $set('hero_eyebrow', 'Conversion Web Partner') }}</span>
        <h1>{!! nl2br(e($set('hero_title', 'Social media bawa perhatian, website bangun kepercayaan.'))) !!}</h1>
        <p class="desc">{{ $set('hero_desc') }}</p>
        <div class="hero-cta">
          <a href="{{ $wa }}" class="btn btn-grad" target="_blank" rel="noopener">Mulai Konsultasi<svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M5 12h14M13 6l6 6-6 6"/></svg></a>
          <a href="#showcase" class="btn btn-line">Lihat Website</a>
        </div>
  ...
  <section class="stats">
    <div class="wrap">
      ...
      <div class="stats-grid reveal">
        <div class="stat"><b class="grad-text">{{ $set('stat1_value','4.000+') }}</b><span>{{ $set('stat1_label','Creator multi-platform') }}</span></div>
        <div class="stat"><b class="grad-text">{{ $set('stat2_value','500+') }}</b><span>{{ $set('stat2_label','Seller partnership') }}</span></div>
        <div class="stat"><b class="grad-text">{{ $set('stat3_value','2.000+') }}</b><span>{{ $set('stat3_label','TAP product collaboration') }}</span></div>
      </div>
    </div>
  </section>
    ...
    <div class="wrap">
      ...
      <div class="syn-grid">
        <div class="syn reveal">
          ...
          <ul>
            <li>{!! $check !!}Konten & kampanye sosial media</li>
            <li>{!! $check !!}Jaringan creator & seller partner</li>
            <li>{!! $check !!}Awareness & traffic ke brand</li>
          </ul>
        </div>
        ...
        <div class="syn reveal" style="transition-delay:.08s">
          ...
          <ul>
            <li>{!! $check !!}Website & landing page premium</li>
            <li>{!! $check !!}Katalog & funnel order WhatsApp</li>
            <li>{!! $check !!}Dukungan teknis & SEO dasar</li>
          </ul>
        </div>
      </div>
    </div>
    ...
    <div class="wrap">
      ...
      <div class="svc-grid">
        ...
        <div class="svc reveal" style="transition-delay:{{ ($i%3)*.06 }}s">
          <div class="svc-ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round">{!! $svcIcons[$svc->icon] ?? $svcIcons['web'] !!}</svg></div>
          <h3>{{ $svc->title }}</h3>
          <p>{{ $svc->description }}</p>
        </div>
        @endforeach
  ...
  <!-- SHOWCASE / LIHAT WEBSITE -->
  @if($showcases->count())
  <section class="section" id="showcase">
    <div class="wrap">
      <div class="sec-head center reveal">
        <span class="eyebrow"><span class="pip"></span> Portofolio Website</span>
        <h2 class="heading">Lihat hasil website yang sudah kami buat</h2>
        <p class="lead">Klik <strong>Lihat</strong> untuk membuka preview, lalu konsultasikan website serupa untuk brand-mu.</p>
      </div>
      @if($categories->count() > 1)
      <div class="filter-row reveal" id="sc-filter">
        <a class="active" data-cat="all">Semua</a>
        @foreach($categories as $c)<a data-cat="{{ $c }}">{{ $c }}</a>@endforeach
      </div>
      @endif
      <div class="sc-grid" id="sc-grid">
        @foreach($showcases as $sc)
        <article class="sc reveal" data-cat="{{ $sc->category }}">
          <div class="sc-thumb" @if($sc->thumbnail) style="background-image:url('{{ asset('storage/'.$sc->thumbnail) }}')" @endif>
            @if($sc->is_featured)<span class="badge">Unggulan</span>@endif
          </div>
          <div class="sc-body">
            @if($sc->category)<span class="cat">{{ $sc->category }}</span>@endif
            <h3>{{ $sc->title }}</h3>
            <div class="sc-foot">
              @if($sc->preview_url)<a href="{{ $sc->preview_url }}" target="_blank" rel="noopener" class="btn btn-line btn-sm">Lihat</a>@endif
              <a href="{{ $wa }}" target="_blank" rel="noopener" class="btn btn-grad btn-sm">Konsultasi</a>
            </div>
          </div>
        </article>
        @endforeach
  ...
  <!-- PACKAGES (NO PRICE) -->
  @if($packages->count())
  <section class="section services" id="paket">
    <div class="wrap">
      <div class="sec-head center reveal">
      ...
      <div class="pkg-grid">
        ...
        <div class="pkg {{ $p->is_featured ? 'feat' : '' }} reveal">
          @if($p->badge)<span class="badge">{{ $p->badge }}</span>@endif
          <div class="name">{{ $p->name }}</div>
          <h3>{{ $p->title }}</h3>
          <p>{{ $p->description }}</p>
          <ul>
            @foreach(($p->features ?? []) as $f)<li>{!! $check !!}{{ $f }}</li>@endforeach
          </ul>
          <a href="{{ $wa }}" class="btn {{ $p->is_featured ? 'btn-grad' : 'btn-line' }} btn-block" target="_blank" rel="noopener">Konsultasi</a>
        </div>
        @endforeach
      </div>
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="feat-grid">
        <div class="feat-card reveal"><div class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="3" width="20" height="14" rx="2"/><line x1="2" y1="8" x2="22" y2="8"/></svg></div><h3>Website / Landing Page</h3><div class="chips"><span>Home</span><span>Profil Usaha</span><span>Produk / Menu</span><span>Keunggulan</span><span>Gallery</span><span>Testimoni</span><span>Lokasi & Maps</span></div></div>
        <div class="feat-card reveal" style="transition-delay:.06s"><div class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><rect x="3" y="3" width="18" height="18" rx="2"/><path d="M3 9h18M9 21V9"/></svg></div><h3>Dashboard Admin Basic</h3><div class="chips"><span>Profil</span><span>Produk / Menu</span><span>Promo</span><span>Gallery</span><span>Testimoni</span><span>FAQ</span><span>Kontak & Sosmed</span></div></div>
        <div class="feat-card reveal"><div class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><path d="M21 11.5a8.4 8.4 0 0 1-12.2 7.5L3 21l2-5.8A8.5 8.5 0 1 1 21 11.5z"/></svg></div><h3>WhatsApp Funnel</h3><div class="chips"><span>Tombol WA utama</span><span>Floating WA</span><span>Link chat otomatis</span></div></div>
        <div class="feat-card reveal" style="transition-delay:.06s"><div class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="1.8" stroke-linecap="round" stroke-linejoin="round"><circle cx="11" cy="11" r="8"/><path d="m21 21-4.3-4.3"/></svg></div><h3>SEO Basic</h3><div class="chips"><span>Meta title & description</span><span>Struktur ramah Google</span><span>Favicon</span><span>Sitemap</span><span>Siap di-index</span></div></div>
      </div>
    </div>
  </section>
  ...
  <section class="section process">
    <div class="wrap">
      ...
      <div class="proc-grid">
        ...
        @foreach($proc as $i => $p)<div class="proc reveal" style="transition-delay:{{ $i*.06 }}s"><div class="proc-num">{{ sprintf('%02d',$i+1) }}</div><h3>{{ $p[0] }}</h3><p>{{ $p[1] }}</p></div>@endforeach
      </div>
    </div>
  </section>
  ...
  <section class="section">
    <div class="wrap">
      ...
      <div class="addons reveal">
        @foreach($addons as $a)<span>{{ $a->name }}</span>@endforeach
      </div>
    </div>
  </section>
  @endif
  ...
  <section class="section" style="padding-top:0">
    <div class="wrap">
      <div class="quote-wrap reveal">
        ...
        <blockquote>{{ $set('quote', 'Di era biaya digital dan potongan platform yang terus meningkat, memiliki website bukan lagi pilihan — melainkan kebutuhan.') }}</blockquote>
  ...
  <section class="section" style="padding-top:0">
    <div class="wrap">
      <div class="cta-wrap reveal">
        <h2>{{ $set('cta_title', 'Saatnya UMKM punya kehadiran digital yang nyata') }}</h2>
        <p>{{ $set('cta_desc') }}</p>
        <div class="cta-actions">
          <a href="{{ $wa }}" class="btn btn-light" target="_blank" rel="noopener">Bertumbuh Bersama Kami</a>
          <a href="#showcase" class="btn btn-outline-light">Lihat Website</a>
        </div>
      </div>
    </div>
  </section>
      ...
      <div class="contact-grid">
        <div class="reveal">
          ...
          <div class="hero-cta" style="margin-top:26px">
            <a href="{{ $wa }}" class="btn btn-grad" target="_blank" rel="noopener">Chat WhatsApp</a>
            <a href="mailto:{{ $email }}" class="btn btn-line">Kirim Email</a>
          </div>
        </div>
        <div class="contact-card reveal" style="transition-delay:.08s">
          <div class="row"><div class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M20 21v-2a4 4 0 0 0-4-4H8a4 4 0 0 0-4 4v2"/><circle cx="12" cy="7" r="4"/></svg></div><div><strong>{{ $set('contact_name','Agung Ando') }}</strong><span>Seamedia Indonesia</span></div></div>
          <div class="row"><div class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><rect x="2" y="4" width="20" height="16" rx="2"/><path d="m22 7-10 6L2 7"/></svg></div><div><strong>{{ $email }}</strong><span>Email resmi</span></div></div>
          <div class="row"><div class="ic"><svg viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 10c0 7-9 12-9 12s-9-5-9-12a9 9 0 0 1 18 0z"/><circle cx="12" cy="10" r="3"/></svg></div><div><strong>{{ $set('contact_location','Palembang, Indonesia') }}</strong><span>Melayani seluruh Indonesia</span></div></div>
        </div>
      </div>
    </div>
  </section>
  ...
  <footer>
    <div class="wrap">
      <div class="foot-grid">
        ...
        <div class="foot-col"><h4>Kontak</h4><a href="mailto:{{ $email }}">{{ $email }}</a><a href="{{ $wa }}" target="_blank" rel="noopener">WhatsApp</a><a href="https://conweb.id" target="_blank" rel="noopener">ConWeb.id</a></div>
      </div>
      <div class="foot-bottom"><p>© {{ date('Y') }} Seamedia ConWeb. Conversion Web Collaboration.</p><p>Dibuat dengan ❤️ untuk UMKM Indonesia</p></div>
    </div>
  </footer>
  ...
  <script>
    ...
    const scF = document.getElementById('sc-filter');
    if (scF) scF.addEventListener('click', e => {
      const link = e.target.closest('a'); if(!link) return;
      scF.querySelectorAll('a').forEach(a => a.classList.remove('active')); link.classList.add('active');
      const cat = link.dataset.cat;
      document.querySelectorAll('#sc-grid .sc').forEach(c => c.style.display = (cat==='all'||c.dataset.cat===cat)?'':'none');
    });
